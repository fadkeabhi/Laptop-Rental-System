...
const Product2 = ({ item, onDelete }) => {
  ...
  return (<><div className="bg-white flex items-center hover:shadow-2xl pb-8 relative mb-4 rounded-md hover:scale-105 transition-all duration-300 shadow-lg lg:w-[24%]  md:w-[40%] sm:w-[80%] overflow-hidden box-border"></div>
    <div className="flex justify-center flex-col items-center gap-2  p-5  box-border">
      <h2 className="text-lg font-bold"> {item.title.length > 15
            ? item.title.substring(0, 20) + " ..."
            : item.title}</h2>
      <p className="text-xs opacity-90 " > {item.description.length > 40
